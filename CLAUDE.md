...
- Rule mutations use server-side GraphQL (`createTransactionRuleV2`, `updateTransactionRuleV2`, `deleteTransactionRule`) because the community client lacks them; they have not been live-validated here. Update clears omitted actions and reportedly ignores input without a merchant, statement, or amount criterion, so edits reread the rule, resend it with changes merged, and reject criteria-less results. `setMerchantAction` is written as a merchant name, not an ID. An all-null `errors` object is a rejection. `deleted: false` is returned even on success, so deletion is confirmed by rereading.
